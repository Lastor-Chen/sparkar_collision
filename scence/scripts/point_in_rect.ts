...
export function runPointInRect(asset) {
  ...
  Helper.checkHit3D(userBBox, planeBBox).monitor().subscribe(res => {
...
class Helper {
  /** 碰撞偵測, 點包含法 */
  static checkHit3D(rectA: BoundingBox3D, rectB: BoundingBox3D) {
   ...
   */
  static pointInRect(point: PointSignal, rect: BoundingBox3D) {
